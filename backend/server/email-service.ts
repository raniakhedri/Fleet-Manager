// Email service with optional nodemailer support
...
async function initializeEmailService() {
  try {
    ...
    console.warn('[EMAIL] Nodemailer not installed. Email functionality will be disabled.');
    console.warn('[EMAIL] To enable emails, install nodemailer from the project root directory');
  }
}

// Initialize on module load
initializeEmailService().catch(() => {
  // Silently fail if nodemailer is not available
});
...
export async function sendDriverCredentialsEmail(
  ...
  if (!transporter || !nodemailer) {
    console.warn('[EMAIL] Skipping email - nodemailer not installed');
  ...
  const mailOptions = {
    from: 'FleetGuard <rannniakhedri@gmail.com>',
    to: driverEmail,
      ...
      <body>
        <div class="container">
          ...
          <div class="content">
            ...
            <p>Vous pouvez maintenant accéder au portail chauffeur pour consulter vos missions assignées, suivre votre progression et gérer votre profil.</p>
            ...
            <center>
              <a href="http://localhost:5173/login" class="button">Se connecter à FleetGuard</a>
            </center>
            
            <div class="footer">
              <p>Si vous avez des questions, veuillez contacter votre administrateur de flotte.</p>
    ...
    `,
    text: `
      Bienvenue à FleetGuard, ${driverName}!
      ...
      Connexion à : http://localhost:5173/login
      
      Si vous avez des questions, veuillez contacter votre administrateur de flotte.
    `,
  };

  try {
    await transporter.sendMail(mailOptions);
    console.log(`[EMAIL] Credentials sent to ${driverEmail}`);
  } catch (error) {
